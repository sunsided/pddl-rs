...
use crate::parsers::{parse_function_symbol, parse_term, space_separated_list0};
...
/// # use pddl::types::{Name, FunctionTerm, Variable, FunctionSymbol, Term};
...
pub fn parse_function_term(input: &str) -> IResult<&str, FunctionTerm> {
    let (remaining, (symbol, terms)) = delimited(tag("("), tuple((parse_function_symbol, space_separated_list0(parse_term))), tag(")"))(input)?;
